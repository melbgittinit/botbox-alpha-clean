...
function resultFor(botId: string, prompt: string) {
...
export async function POST(request: Request) {
  ...
  if (typeof botId !== "string" || !BOT_IDS.has(botId)) {
  ...
  const response = NextResponse.json({
    ...
    botId,
    result: resultFor(botId, cleaned),
    notice: "Server-backed alpha test. No external AI model, customer profile, checkout, or persistent prompt storage is used here.",
  });
