...
export default function CreatorDeskPage(){
  ...
  const finishedNonBrands=creations.filter(i=>i.status==="COMPLETE"&&i.kind!=="brand").length;
  ...
  return <main className="cc4-shell">
    ...
    <section className="cc4-panel cc4-next"><div><span className="cc4-eyebrow">NEXT BEST ACTION</span><h2>{active?`Finish ${active.title}`:finishedNonBrands>=2?"You may have a brand here":recent?`Build from ${recent.title}`:"Start your first creation"}</h2><p>{active?`You’re ${active.progress}% complete. Go directly back to the work.`:finishedNonBrands>=2?`You have ${finishedNonBrands} finished creations. Brand Hall can help organize them under one recognizable identity.`:recent?"You already did the hard part. Reuse what works.":"Start with Creator Compass or choose a builder."}</p></div><a className="cc4-primary" href={active?active.route:finishedNonBrands>=2?"/creator-college/brand-hall":recent?recent.route:"/creator-college"}>{active?"CONTINUE MY CREATION":finishedNonBrands>=2?"OPEN BRAND HALL":recent?"BUILD FROM THIS":"START CREATING"}</a></section>
    ...
    <section className="cc4-section"><div className="cc4-section-head"><div><span className="cc4-eyebrow">QUICK ACTIONS</span><h2>WHAT DO YOU WANT TO MAKE?</h2></div></div><div className="cc4-grid cc4-actions-grid"><a href="/creator-college/start">⚡<b>SMART START</b><small>Use my saved audience, goals and brand</small></a><a href="/creator-college/video">🎥<b>5-VIDEO SERIES</b><small>Plan scripts, visuals and posting</small></a><a href="/creator-college/event">⛪<b>CHURCH / GROUP EVENT KIT</b><small>Plan, promote, run and follow up</small></a><a href="/creator-college/service">💼<b>SIMPLE SERVICE OFFER</b><small>Create to Earn: package a skill into a sellable service</small></a><a href="/creator-college/brand-hall">◇<b>BRAND HALL</b><small>Organize creations into recognizable brands</small></a><a href="/creator-college/profile">◎<b>MY CREATOR PROFILE</b><small>Update voice, look, audience and brands</small></a></div></section>
