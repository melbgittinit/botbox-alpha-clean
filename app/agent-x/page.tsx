...
export default function AgentXPage() {
  ...
  const button = {
    display: 'inline-block', padding: '14px 20px', borderRadius: 999, textDecoration: 'none', fontWeight: 800, letterSpacing: '.03em'
  } as const;
      ...
      <section id="agents" style={{padding:'54px 24px'}}>
        <div style={{maxWidth:1180,margin:'0 auto'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:16}}>
            ...
            ].map(([name,type,copy]) => <article key={name} style={card}><div style={{fontSize:12,fontWeight:800,letterSpacing:'.14em',color:'#79b8ff'}}>{type}</div><h3 style={{fontSize:25,margin:'12px 0 10px'}}>{name}</h3><p style={{color:'#aebaca',lineHeight:1.55,margin:0}}>{copy}</p></article>)}
          </div>
        </div>
      </section>

      <section style={{padding:'18px 24px 58px'}}><div style={{maxWidth:1180,margin:'0 auto'}}><div style={{fontSize:13,fontWeight:800,letterSpacing:'.2em',color:'#79b8ff'}}>READY-MADE WORKFORCES</div><div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(245px,1fr))',gap:16,marginTop:18}}>{[
        ['Small Business X','Your first AI department.'],['Beauty Business X','Your talent built the business. Agent X builds the system behind it.'],['Restaurant X','Great food deserves great systems.'],['Creator Business X','Your creativity deserves a company.']
      ].map(([name,copy]) => <article key={name} style={card}><h3 style={{fontSize:27,margin:'0 0 10px'}}>{name}</h3><p style={{color:'#aebaca',lineHeight:1.55,margin:0}}>{copy}</p></article>)}</div></div></section>
      ...
      <section id="build-my-team" style={{padding:'62px 24px',background:'#09101a'}}>
        <div style={{maxWidth:980,margin:'0 auto'}}>
          ...
          {activation && <section style={{marginTop:24,display:'grid',gap:16}}>
            ...
            <div style={card}><div style={{fontSize:12,letterSpacing:'.18em',color:'#79b8ff',fontWeight:800}}>FIRST REPORT CREATED</div><h3 style={{fontSize:27,margin:'10px 0'}}>{activation.report.title}</h3><p style={{color:'#aebaca'}}>Your assessment, workforce assignment and Mission 001 are now stored in the Agent X preview system. The next build step is the full customer Command Center and mission approval screen.</p></div>
          </section>}
        </div>
      </section>

      <section style={{padding:'62px 24px'}}><div style={{maxWidth:980,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(250px,1fr))',gap:18}}><div style={card}><div style={{fontSize:13,color:'#79b8ff',fontWeight:800}}>MISSION</div><h3>Mission-Based Work</h3><p style={{color:'#aebaca'}}>Define the objective, assign agents, surface approvals, track progress and produce a usable report.</p></div><div style={card}><div style={{fontSize:13,color:'#79b8ff',fontWeight:800}}>CONTROL</div><h3>Human Approval</h3><p style={{color:'#aebaca'}}>Agent X recommends and prepares work while people retain authority for decisions and high-impact actions.</p></div><div style={card}><div style={{fontSize:13,color:'#79b8ff',fontWeight:800}}>REPORT</div><h3>What Happened + What Next</h3><p style={{color:'#aebaca'}}>Every mission should end with a clear intelligence report and the next useful opportunity.</p></div></div></section>
    </main>
  );
}
