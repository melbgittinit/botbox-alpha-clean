...
export default function Interview(){
 ...
 useEffect(()=>{const p=new URLSearchParams(window.location.search);const t=p.get("topic");if(t)setTopic(t);},[]);
 async function run(){setLoading(true);try{const r=await fetch("/api/media/interview",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({topic,question,target_length:seconds})});setResult(await r.json());}finally{setLoading(false);}}
 ...
 </div></div><div className={styles.panel}>{result?<><span className={styles.status}>{result.escalation_required?"!":"✓"} {result.authorization_status}</span><div className={styles.quote} style={{marginTop:16}}>{result.response}</div><p className={styles.small}>Source as: The Bot Stores Official Information Director™, AI media representative.</p>{result.verify_url&&<p><Link className={styles.link} href={result.verify_url}>Verify Media Use Receipt →</Link></p>}<p className={styles.small}>Receipt: {result.receipt_id}</p><p className={styles.small}>Paid advertising, voice cloning and synthetic alteration are not authorized by this alpha receipt.</p></>:<p>Ask a question to generate a Truth Vault-grounded media response.</p>}</div></section>
 </div></main>}