...
export default function Pitch(){
 ...
 async function run(){setLoading(true);try{const r=await fetch("/api/media/pitch",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({coverage})});setResult(await r.json());}finally{setLoading(false);}}
 ...
 <section className={styles.panel}><div className={styles.form}><label>Your story / beat<textarea rows={5} value={coverage} onChange={e=>setCoverage(e.target.value)}/></label><button className={styles.button} onClick={run} disabled={loading}>{loading?"Looking for fit…":"FIND MY BOT STORES ANGLE"}</button></div>
 {result&&<div className={styles.result}><div className={styles.meta}>{result.strong_fit?"MATCHED":"NO FORCED PITCH"}</div><p>{result.message||result.error}</p></div>}
 {result?.matches?.length>0&&<div className={styles.section}>{result.matches.map((m:any)=><article className={styles.card} key={m.id} style={{marginBottom:12}}><div className={styles.meta}>{m.bot}</div><h2>{m.title}</h2><p>{m.why}</p><p><b>Possible angle:</b> {m.suggested_headline}</p><Link className={styles.link} href={"/press/headlines?story="+m.id}>Open in Headline Bell</Link></article>)}</div>}</section>
 </div></main>
}